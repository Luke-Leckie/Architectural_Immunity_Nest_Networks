...
eff_dynamics  <- lmer(log(efficiency)                ~ treatment * time + (1|colony/subset) + (1|week), data = DF_DATA)
den_dynamics  <- lmer(log(density)                   ~ treatment * time + (1|colony/subset) + (1|week), data = DF_DATA)
...
wd_dynamics   <- lmer(log(weighted_diameter)   ~ treatment + time + (1|colony/subset) + (1|week), data = DF_DATA)
...
en_144<-lmer(sqrt(new_ne_count) ~ treatment+ (1|colony)+(1|week), data = DF_144)
