...
cham_0<- subset(chamber_dat, time==0)
model0<-glmer(variable+0.1  ~ treatment+  (1|colony/subset), family='Gamma',data =cham_0)#No week rf in these models or it does not converge
car::Anova(model0)
summary(model0)
...
nrow(DF_144)
cham_0<- subset(chamber_dat, time==0)
model0<-glmer(variable  ~ treatment+  (1|colony/subset)+(1|week), family='poisson',data =cham_0)#No week rf in these models or it does not converge
car::Anova(model0)
summary(model0)
